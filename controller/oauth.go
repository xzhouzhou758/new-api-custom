...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string) (*model.User, error) {
	...
	if provider.IsUserIDTaken(oauthUser.ProviderUserID) {
		...
		// Update avatar if the provider returned one and it changed
		if avatar, ok := oauthUser.Extra["avatar"].(string); ok && avatar != "" && user.Avatar != avatar {
			user.Avatar = avatar
	...
	if avatar, ok := oauthUser.Extra["avatar"].(string); ok && avatar != "" {
		user.Avatar = avatar
	}
	if oauthUser.Email != "" {
