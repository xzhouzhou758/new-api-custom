                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <SettingsSwitchItem className='lg:col-span-2'>
                      <SettingsSwitchContent>
                        <FormLabel>
                          {t('Enable guild & role verification')}
                        </FormLabel>
                        <FormDescription>
                          {t(
                            'When enabled, only members of the required server holding at least one required role can sign in. Requires the privileged guilds.members.read scope.'
                          )}
                        </FormDescription>
                      </SettingsSwitchContent>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder={t('Discord server ID, e.g. 123456789012345678')}
                          autoComplete='off'
                          disabled={!form.watch('discord.guild_verify_enabled')}
                          value={field.value ?? ''}
                          onChange={(event) =>
                            field.onChange(event.target.value)
                          }
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder={t('Comma separated, e.g. 111111111111,222222222222')}
                          autoComplete='off'
                          disabled={!form.watch('discord.guild_verify_enabled')}
                          value={field.value ?? ''}
                          onChange={(event) =>
                      ...
                      <FormDescription>
                        {t(
                          'Comma separated role IDs. When empty, only guild membership is required.'
                        )}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
